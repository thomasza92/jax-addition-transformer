...
from blocks.transformer import count_parameters, init_transformer_model
from config import debug_config, final_10m_config
...
def print_examples(params, config):
  for a, b in [
    ...
    tqdm.write(
        f"  {a:03d}+{b:03d}= " f"predicted {pred} " f"expected {expected}"
    )
